modAds.grid.Items = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'modads-grid-items';
	}
	Ext.applyIf(config, {
	...
	modAds.grid.Items.superclass.constructor.call(this, config);
...
Ext.extend(modAds.grid.Items, MODx.grid.Grid, {
	windows: {},
...
Ext.reg('modads-grid-items', modAds.grid.Items);